        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          {categories.map((cat) => (
            <Link
              key={cat.name}
              to={cat.href}
              className="bg-brand-ghost border border-brand-mist rounded-xl p-5 pt-5 pb-5 transition-colors duration-100 hover:border-brand-violet hover:bg-brand-lavender group"
            >
              <div className="w-10 h-10 bg-brand-violet rounded-lg mb-2.5" />
              <p className="font-heading font-bold text-[15px] text-brand-ink">{cat.name}</p>
              <p className="font-body text-[13px] text-brand-muted-text mt-1">{cat.desc}</p>
            </Link>
          ))}
        </div>
      </div>
