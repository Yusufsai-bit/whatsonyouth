...
export function OpportunityCard({ category, title, organisation, location, dateInfo, communityBadge, href }: OpportunityCardProps) {
  return (
    <div className="bg-white border border-brand-mist rounded-xl p-5 flex flex-col">
      <span className="inline-block self-start bg-brand-lavender text-brand-violet font-body font-medium text-xs rounded-full px-2.5 py-[3px]">
        {category}
      </span>
      <h3 className="font-heading font-bold text-lg text-brand-ink mt-2">{title}</h3>
      <p className="font-body text-sm text-brand-muted-text mt-1">{organisation}</p>
      <p className="font-body text-[13px] text-brand-muted-text mt-1">{location} · {dateInfo}</p>
      {communityBadge && (
        <span className="inline-block self-start bg-brand-ghost border border-brand-mist text-brand-muted-text font-body text-[11px] rounded-full px-2 py-[2px] mt-2">
          Submitted by community
        </span>
      )}
      <Link
        to={href || '#'}
        className="font-body font-medium text-sm text-brand-violet mt-3 hover:underline"
      >
        View details →
      </Link>
    </div>
...
export default function FeaturedOpportunities() {
  return (
    <section className="bg-brand-ghost px-6 py-12 md:px-16 md:py-16">
      <div className="max-w-7xl mx-auto">
        <h2 className="text-[28px] text-brand-ink">Featured opportunities</h2>
