...
const impactStats = [
  { value: '10,000+', label: 'monthly visits' },
  { value: '500+', label: 'young people signed up' },
...
export default function Footer() {
  return (
    <footer className="bg-brand-dark">
      {/* Impact strip */}
      <div className="border-b border-brand-footer-divider px-6 py-8 md:px-16 md:py-10">
        <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6">
          {impactStats.map(s => (
            <div key={s.label} className="text-center md:text-left">
              <div className="font-heading font-bold text-2xl md:text-3xl text-white leading-tight">{s.value}</div>
              <div className="font-body text-[13px] text-brand-footer-text mt-1">{s.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="px-6 py-10 md:px-16 md:py-12">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          <div>
            ...
            <p className="font-body text-[13px] text-brand-footer-text mt-3">✨ New listings every day</p>
          </div>
