...
export default function AdminEditListing() {
  ...
  if (loading) return <AdminLayout><AdminHeader title="Edit listing" showAddButton={false} /><div className="p-8 font-body text-sm text-brand-text-muted">Loading...</div></AdminLayout>;
        ...
        <form onSubmit={handleSubmit} className="bg-white border border-brand-card-border rounded-xl p-8 max-w-[800px]">
          ...
          <div className="mt-5">
            ...
            {imagePreview ? (
              ...
              <div onClick={() => fileInputRef.current?.click()} className="bg-brand-section-alt border-2 border-dashed border-brand-input-border rounded-xl p-8 text-center cursor-pointer hover:border-brand-violet transition-colors">
          ...
          <button type="submit" disabled={submitting} className="mt-6 bg-brand-coral text-white font-heading font-bold text-base rounded-lg px-7 py-3.5 transition-colors hover:bg-brand-coral-light disabled:opacity-50">
            {submitting ? 'Saving...' : 'Save changes'}
          </button>
        </form>

        <button onClick={() => setShowDeleteModal(true)} className="font-body text-sm text-brand-coral hover:underline mt-4 block">
          Delete this listing
        </button>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-2xl p-8 max-w-[400px] w-full mx-4">
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setShowDeleteModal(false)} className="flex-1 border-2 border-brand-dark text-brand-dark font-heading font-bold text-sm rounded-lg py-2.5">Cancel</button>
              <button onClick={handleDelete} className="flex-1 bg-[#E24B4A] text-white font-heading font-bold text-sm rounded-lg py-2.5">Delete listing</button>
            </div>
          </div>
        </div>
      )}
